...
async function loadSurahs() {
    ...
    } catch (e) { if (list) list.innerHTML = '<p class="text-center text-slate-400 py-10">কুরআন লোড করা সম্ভব হয়নি।</p>'; }
}
...
window.loadAyahs = async function(num, name) {
    ...
    try {
        ...
        `).join('');
        document.querySelector('main').scrollTop = 0;
    } catch (e) { showToast('আয়াত লোড করা সম্ভব হয়নি'); }
};
...
function startCompass(qiblaDeg) {
    ...
    const dot = document.querySelector('.qibla-dot');
    const pointer = document.getElementById('qibla-pointer-group');
    ...
    const handleOrientation = (e) => {
        ...
        let diff = Math.abs(heading - qiblaDeg);
        if (diff > 180) diff = 360 - diff;

        if (diff < 5) {
            dot?.classList.add('compass-aligned');
            pointer?.classList.add('text-emerald-500');
        } else {
            dot?.classList.remove('compass-aligned');
            pointer?.classList.remove('text-emerald-500');
        }
    };
